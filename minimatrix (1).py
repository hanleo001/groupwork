...
class Matrix:
	...
	def reshape(self, newdim):
		...
		if newdim[0]*newdim[1]!=self.dim[0]*self.dim[1]:
			raise ValueError
		newmatirx=Matrix(dim=newdim)
		index=0
		coloum=newdim[1]
		for i in self.data:
			for j in i:
				a,b=divmod(index,coloum)
				index+=1
				newmatirx.data[a][b]=j
		return newmatirx
	...
	def T(self):
		...
		Examples:
			...
			>>> [[1 4]
				 ...
				 [3 6]]
		"""
		newmatrix=Matrix(dim=(self.dim[1],self.dim[0]))
		
		for i,row in enumerate(self.data):
			for j,coloum in enumerate(row):
				newmatrix.data[j][i]=coloum
		return newmatrix
	...
	def copy(self):
		...
		Returns:
			Matrix: 一个self的备份
		"""
		newmatrix=Matrix(dim=self.dim)
		for i,row in enumerate(self.data):
			for j,colome in enumerate(row):
				newmatrix.data[i][j]=colome
		return newmatrix
...
def nrandom(dim):
	...
	for i in range(dim[0]):
		for j in range(dim[1]):
			res.data[i][j]=random.randint(-2**31,2**31)
	return res
